...
- The desktop session-switch check verified distinct session ids, ran `echo FINAL_SWITCH_SECOND`, switched back, restored `FINAL_DESKTOP`, restored the first session's uploaded image draft, kept the second session initially clean, restored the second session's unsent draft only in that second session, and reset ArrowUp history recall to the returned session's latest command.
